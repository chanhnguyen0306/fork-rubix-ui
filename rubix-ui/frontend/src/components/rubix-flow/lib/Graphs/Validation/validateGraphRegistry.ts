...
export const validateGraphRegistry = (
  graphRegistry: GraphRegistry
): string[] => {
  const errorList: string[] = [];
  graphRegistry.nodes.nodeTypeNameToNodeFactory.forEach(
    (nodeFactory, nodeTypeName) => {
      ...
      node.inputSockets.forEach((socket) => {
        ...
        const valueType = graphRegistry.values.valueTypeNameToValueType.get(
          socket.valueTypeName
        );
