...
func (inst *App) AddLocation(connUUID string, body *model.Location) *model.Location {
	if body.Name == "" {
...
func (inst *App) GetLocations(connUUID string) (resp []model.Location) {
	resp = []model.Location{}
	client, err := inst.getAssistClient(&AssistClient{ConnUUID: connUUID})
	if err != nil {
		inst.uiErrorMessage(fmt.Sprintf("error %s", err.Error()))
...
func (inst *App) GetLocation(connUUID string, uuid string) *model.Location {
	client, err := inst.getAssistClient(&AssistClient{ConnUUID: connUUID})
	if err != nil {
		inst.uiErrorMessage(fmt.Sprintf("error %s", err.Error()))
		return nil
...
func (inst *App) UpdateLocation(connUUID string, uuid string, host *model.Location) *model.Location {
	client, err := inst.getAssistClient(&AssistClient{ConnUUID: connUUID})
	if err != nil {
		inst.uiErrorMessage(fmt.Sprintf("error %s", err.Error()))
		return nil
